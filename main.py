...
class ZScoreIcebergBot:
    ...
    def _run_main_loop(self) -> None:
        """Event-driven: WS callbacks → strategy.on_tick(), 1s health only."""
        logger.info("=" * 80)
        ...
        while self.running:
            try:
                # Minimal 1s health ping + position management only
                now_sec = time.time()
                if now_sec - last_health_sec >= 1.0:
                    self._check_stream_health()
                    
                    # Score decay exit check for open positions
                    if self.strategy.current_position is not None:
                        current_price = self.data_manager.get_last_price()
                        if current_price > 0:
                ...
                time.sleep(0.1)  # Minimal CPU - WS callbacks handle real-time updates
