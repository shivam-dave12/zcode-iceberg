...
# Replace the runmainloop() with event-driven callbacks:

    def _run_main_loop(self) -> None:
        """Event-driven: WS callbacks trigger strategy.update(), 1s health only."""
        logger.info("=" * 80)
        logger.info("BOT RUNNING - EVENT-DRIVEN Z-SCORE HUNTER ACTIVE")
        logger.info("=" * 80)
        ...
        while self.running:
            try:
                # Minimal 1s health ping only
                now_sec = time.time()
                if now_sec - last_health_sec >= 1.0:
                    self.check_stream_health()
                    self.strategy.manage_open_position(  # Score decay exit check
                        datamanager=self.datamanager,
                        ordermanager=self.ordermanager,
                        riskmanager=self.riskmanager,
                        currentprice=self.datamanager.get_last_price()
                    )
                    last_health_sec = now_sec
                
                time.sleep(0.1)  # Minimal CPU
