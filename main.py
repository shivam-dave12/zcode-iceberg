...
class ZScoreIcebergBot:
    ...
    def __init__(self, controller=None) -> None:
        ...
        self._last_strategy_tick_sec: float = 0.0
    ...
    def start(self) -> None:
        ...
        try:
            ...
            # Attach event-driven callbacks from WebSocket to strategy
            self._attach_event_driven_callbacks()
    ...
    def _attach_event_driven_callbacks(self) -> None:
        """
        Attach WebSocket callbacks to drive strategy.on_tick in an
        event-driven fashion (orderbook/trade updates).

        Uses a lock + time gate to avoid re-entrancy and excessive CPU.
        """
        ws = getattr(self.data_manager, "ws", None)
        if ws is None:
            logger.warning(
                "DataManager WebSocket handle is None; "
                "cannot attach event-driven callbacks."
            )
            return

        if self._callbacks_attached:
            logger.info("Event-driven callbacks already attached; skipping")
            return

        def _wrap(cb):
            # simple wrapper to keep reference if needed later
            return cb

        ws.orderbook_callbacks.append(_wrap(self._on_ws_market_update))
        ws.trades_callbacks.append(_wrap(self._on_ws_market_update))

        self._callbacks_attached = True
        logger.info(
            "Event-driven callbacks attached: orderbook + trades -> strategy.on_tick"
        )

    def _on_ws_market_update(self, data: dict) -> None:
        """
        WebSocket callback invoked on each orderbook/trade update.

        - Throttled to at most ~20 Hz.
        - Protected by a non-blocking lock to avoid re-entrancy.
        """
        if not self.running:
            return

        now = time.time()
        # Throttle to at most 20 strategy ticks per second
        if now - self._last_strategy_tick_sec < 0.05:
            return

        if not self._tick_lock.acquire(blocking=False):
            # Another tick is in progress; skip to avoid piling up
            return

        try:
            self._last_strategy_tick_sec = now
            ...
            logger.error(
                f"Error in strategy.on_tick from WS callback: {e}", exc_info=True
            )
        finally:
    ...
    def _run_main_loop(self) -> None:
        ...
        - Strategy ticks are driven by WebSocket callbacks (no 120 ms poll).
        - Main loop performs 1-second heartbeat: stream health + Telegram report.
        """
        logger.info("=" * 80)
        ...
        while self.running:
            try:
                ...
                time.sleep(1.0)  # 1-second heartbeat only
            except Exception as e:
                logger.error(f"Error in main loop: {e}", exc_info=True)
                time.sleep(1.0)

    # ======================================================================
    # Stream health & restart
    # ======================================================================
            ...
            for attempt in range(3):
                try:
                    ...
                    if self.data_manager.start():
                        ...
                        # Re-attach event-driven callbacks to new ws instance
                    ...
                    logger.error(
                        f"Error in restart attempt {attempt + 1}: {e}", exc_info=True
                    )
                    time.sleep(5.0)

            if not restart_success:
                error_msg = (
                    "❌ WEBSOCKET RESTART FAILED\n"
                    "Could not restore connection after 3 attempts"
                )
                try:
    ...
    def _maybe_send_telegram_report(self) -> None:
        ...
        try:
            ...
            if last_price > 0:
                ...
                if atr is not None:
                    price_line += f" | ATR{config.ATR_WINDOW_MINUTES}m: {atr*100:.3f}%"
                lines.append(price_line)

            if balance_info:
                lines.append(
                    f"Balance: {balance_info.get('available', 0.0):.2f} USDT"
                )

            if pos:
                dur_min = (now - pos.entry_time_sec) / 60.0
                lines.append(
                    f"Position: {pos.side.upper()} {pos.quantity:.3f} "
                    f"@ {pos.entry_price:.2f} ({dur_min:.1f}min)"
                )
